...
if [ -z "$1" ]; then
    echo "usage: ./movie_preview.sh VIDEO OUTPUT"
    exit
fi
if [ -z "$2" ]; then
    echo "usage: ./movie_preview.sh VIDEO OUTPUT"
...
#OUT_DIR=`pwd`
#OUT_DIR="/var/www/GoPro/thumbnail"
#HEIGHT=$2
#COLS=$3
#ROWS=$4
#OUT_FILENAME=$5
...
HEIGHT=200
#Output is a fixed Width of 350
#WIDTH=350
COLS=60
ROWS=1
#echo $OUT_FILEPATH
...
ASPECT=`ffprobe -show_streams "$MOVIE" 2> /dev/null | grep display_aspect_ratio | head -n1 | sed 's/.*=//'`
...
if [ "$NB_FRAMES" = "N/A" ]; then
    echo "Fallback to ffmpeg for frame counting!"
...
#FFMPEG_CMD="ffmpeg -loglevel panic -i \"$MOVIE\" -y -frames 1 -q:v 1 -vf \"select=not(mod(n\,$NTH_FRAME)),scale=${WIDTH}:-1,tile=${COLS}x${ROWS}\" \"$OUT_FILEPATH\""
